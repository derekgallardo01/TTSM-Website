...
add_action('admin_enqueue_scripts', 'load_theme_scripts');
 function load_theme_scripts() {
	...
	wp_register_script('smposts-sc-js', get_template_directory_uri().'/plugins/shortcode-select-posts/smposts_sc.js', array('spotmassage-all-js'),false,true);
	wp_enqueue_script('smposts-sc-js');
...
function spotmassage_setup() {
    ...
    register_nav_menu( 'header-menu-2', __( 'Header Menu 2', 'spotmassage' ) );

    register_nav_menu( 'header-menu-3', __( 'Header Menu 3', 'spotmassage' ) );

       register_nav_menu( 'header-menu-4', __( 'Header Menu 4', 'spotmassage' ) );
...
function spotmassage_widgets_init() {
    ...
    register_sidebar(array(
        ...
        register_sidebar(array(
        'name'          => __( 'Left Sidebar Van User', 'spotmassage' ),
        'id'            => 'sidebar-left-van-user',
        'description'   => __( 'This sidebar is used in the right section', 'spotmassage' ),
        'class'         => '',
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget' => '</div>',
...
function pre_submission_handler( $form ) {
	//$_SERVER["REMOTE_ADDR"]=='47.9.199.108'
	if(1){
		//print_r($_POST); die();
		
		$getadmin = get_bloginfo('admin_email');
		
		$isSend = wp_mail($getadmin, "Job has submitted on your site", "New job submitted on your site That's the Spot Massage Therapy", '', false);
		//$isSend = wp_mail("gpsbaroli@gmail.com", "Job has submitted on your site", "New job submitted on your site That's the Spot Massage Therapy", '', false);
