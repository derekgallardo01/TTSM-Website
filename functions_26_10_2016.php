...
if(isset($_POST["user_id"]) && $_POST["user_id"]!=''){
	...
	$sorting         = array(  );
	$search_criteria['field_filters'][] = array( 'key' => 'created_by', 'value' => $userid );

$entries         = GFAPI::get_entries( 3, $search_criteria, $sorting );

$lead_id = $entries[0]["id"];
/* global $wpdb;echo $querystr = "    SELECT *     FROM ".$wpdb->prefix."rg_lead_meta    WHERE meta_key = 'user_registration_feed_id' and meta_value = '".$userid."' ";$leads = $wpdb->get_results($querystr, OBJECT);print_r($leads);die(); */

if(isset($_FILES["licence"]) && $_FILES["licence"]["name"]!='' && $lead_id!=''){
	
	$frm = new GFFormsModel();
	$frurl = $frm->upload_file(3,$_FILES["licence"]);
	
	GFAPI::update_entry_field($lead_id, 8, $frurl);
